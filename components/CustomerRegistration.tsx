...
const CustomerRegistration: React.FC<RegistrationProps> = ({ onSaved }) => {
  const [formData, setFormData] = useState({ nome: '', nomeFantasia: '', tel: '', doc: '', end: '', numeroCasa: '', bairro: '', municipio: '', idClient: '' });
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      const dadosParaSalvar = { 
        ...
        endereco: [formData.end.trim(), formData.numeroCasa.trim()].filter(Boolean).join(', '),
  ...
  return (
    <div className="w-full relative">
      ...
      <form onSubmit={handleSubmit} className="space-y-4 md:space-y-6">
        ...
        <FormInput label="NÚMERO DA CASA" id="numeroCasa" value={formData.numeroCasa} onChange={handleChange} />
