...
const OrderManagement: React.FC = () => {
  ...
  const agruparPedidos = (listaReservas: any[], filtrarUrgencia: boolean) => {
    ...
    return Object.values(grupos).sort((a: any, b: any) => new Date(b.dataEvento).getTime() - new Date(a.dataEvento).getTime());
  };

  const pedidosNormais = agruparPedidos(reservas.filter(r => r.status !== 'Solicitado' && r.status !== 'Finalizado'), filtroUrgentes);
  const pedidosWeb = agruparPedidos(reservas.filter(r => r.status === 'Solicitado'), false);

  const handleAceitarPedido = async (pedido: any) => {
      if(!window.confirm(`Aceitar pedido de ${pedido.nomeCliente}? Isso irá reservar o estoque.`)) return;
      try {
          for (const item of pedido.itens) {
            await db.from('reservas').update({ status: 'Pendente' }).eq('id', item.id);
            const prodEstoque = estoque.find(e => e.item === item.item);
            if(prodEstoque) {
                await db.from('estoque').update({
                    disponivel: prodEstoque.disponivel - item.quantidade,
                    reservado: prodEstoque.reservado + item.quantidade
                }).eq('id', prodEstoque.id);
            }
          }
  ...
  const handleRecusarPedido = async (pedido: any) => {
      if(!window.confirm(`RECUSAR e EXCLUIR pedido de ${pedido.nomeCliente}?`)) return;
      try {
          for (const item of pedido.itens) {
            await db.from('reservas').delete().eq('id', item.id);
          }
          alert("Pedido recusado e removido.");
          fetchData();
      } catch (error: any) {
      ...
      <html>
        <head>
          ...
          <style>
            ...
            .contract-container { width: 100%; border: 1px solid #000; padding: 20px; box-sizing: border-box; min-height: 98vh; display: flex; flex-direction: column; }
  ...
  const totalUrgentes = pedidosNormais.filter((r:any) => verificarUrgencia(r.dataDevolucao)).length;
  ...
  return (
    <div className="max-w-7xl mx-auto pb-20 animate-in fade-in duration-700">
      ...
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-8 w-full mt-8 px-4">
        
        {/* COLUNA ESQUERDA (2/3) */}
        <div className="lg:col-span-2 flex flex-col gap-8 items-center">
            {pedidosNormais.length === 0 && (
                <p className="text-gray-400 font-bold italic mt-10">Nenhum pedido em andamento.</p>
            )}
            {pedidosNormais.map((pedido: any, idx: number) => {
                ...
                <div key={idx} className={`w-full rounded-[45px] p-8 border-2 shadow-xl ${atrasado ? 'bg-red-50 border-red-200' : urgente ? 'bg-amber-50 border-amber-200' : 'bg-white border-gray-50'}`}>
                    ...
                    <div className="flex gap-2">
                        <button onClick={() => setModalVipAberto(true)} className="w-10 h-10 bg-green-500 text-white rounded-full flex items-center justify-center hover:bg-green-600 transition-colors"><i className="fa-brands fa-whatsapp"></i></button>
        ...
        <div className="lg:col-span-1">
            <div className="sticky top-4">
                <div className="bg-gradient-to-br from-[#b24a2b] to-[#8a3821] text-white p-4 rounded-t-[30px] shadow-lg text-center">
                    <h3 className="font-black text-lg uppercase italic tracking-wider flex items-center justify-center gap-2">
                         <i className="fa-solid fa-earth-americas"></i> Pedidos Online
                    </h3>
                    <p className="text-[10px] font-bold opacity-80">Solicitações via Web App</p>
                </div>
                
                <div className="bg-white border-x-2 border-b-2 border-gray-100 rounded-b-[30px] p-4 shadow-xl min-h-[400px] max-h-[80vh] overflow-y-auto space-y-4">
                    {pedidosWeb.length === 0 ? (
                        <div className="flex flex-col items-center justify-center py-20 text-gray-300 gap-4">
                            ...
                            <p className="text-xs font-bold uppercase text-center px-8">Nenhuma solicitação nova.</p>
                        </div>
                    ) : (
                        pedidosWeb.map((pWeb: any, idx: number) => (
                             <div key={idx} className="bg-blue-50 p-4 rounded-2xl border border-blue-100 relative group hover:shadow-md transition-all">
                                <div className="absolute top-0 right-0 bg-blue-500 text-white text-[9px] font-black px-3 py-1 rounded-bl-xl uppercase">Novo</div>
                                <h4 className="font-bold text-gray-800 uppercase mb-1 text-sm">{pWeb.nomeCliente}</h4>
                                <p className="text-[10px] text-gray-500 mb-3">{formatarDataBR(pWeb.dataEvento)} • {pWeb.itens.length} itens</p>
                                
                                <div className="bg-white/60 p-2 rounded-lg mb-3">
                                    {pWeb.itens.map((i: any) => (
                                        <div key={i.id} className="flex justify-between text-[10px] text-gray-600 mb-1 border-b border-gray-100 last:border-0 pb-1 last:pb-0">
                                            <span>{i.item}</span>
                                            <span className="font-bold">x{i.quantidade}</span>
                                        </div>
                                    ))}
                                </div>
                                
                                {pWeb.observacoes && (
                                    <p className="text-[9px] text-gray-600 italic mb-3 bg-yellow-50 p-2 rounded border border-yellow-100">Obs: {pWeb.observacoes}</p>
                                )}

                                <div className="flex gap-2">
                                    <button onClick={() => handleAceitarPedido(pWeb)} className="flex-1 bg-green-500 text-white text-[10px] font-black py-2 rounded-lg hover:bg-green-600 shadow-sm">Aceitar</button>
                                    <button onClick={() => handleRecusarPedido(pWeb)} className="px-3 bg-red-100 text-red-500 rounded-lg hover:bg-red-200"><i className="fa-solid fa-trash"></i></button>
                                </div>
                             </div>
                        ))
                    )}
                                ...
                                <div key={idx} className="flex items-center justify-between bg-white p-3 rounded-xl border border-gray-100 shadow-sm">
                                    ...
                                    <div className="flex items-center gap-4">
                                        <div className="flex items-center gap-2 bg-gray-50 px-3 py-1 rounded-lg">
                                            ...
                                            <input type="number" className="w-12 bg-transparent text-center font-black text-sm outline-none" value={item.quantidade} min="1" onChange={(e) => handleAlterarQtdExistente(idx, parseInt(e.target.value))} />
                                        </div>
      ...
      {modalVipAberto && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-sm flex items-center justify-center z-50 p-4">
            <div className="bg-white rounded-[35px] p-10 w-full max-w-md shadow-2xl border border-gray-100 animate-in zoom-in duration-300 text-center">
                <div className="w-20 h-20 bg-blue-50 rounded-full flex items-center justify-center mx-auto mb-6"><i className="fa-solid fa-crown text-3xl text-blue-600"></i></div>
                <h3 className="text-xl font-black text-gray-800 uppercase italic mb-2">Função Bloqueada</h3>
                <p className="text-sm font-bold text-gray-500 mb-8">Essa função está disponível apenas na versão <span className="text-[#b24a2b]">VIP</span>.</p>
                <button onClick={() => setModalVipAberto(false)} className="w-full py-4 bg-red-600 text-white rounded-2xl font-black uppercase text-xs tracking-widest hover:bg-red-700 shadow-lg transition-all">Sair</button>
            </div>
        </div>
      )}
